...
import useStyles from './gmailLoginStyles';
...
const PhoneLogin = () => {
  ...
  const [overlayScreen,enableoverlayScreen] = useState(false);
  ...
  return (
      ...
      <Grid item sm={12}>
      <Button className={clsx(classes.button, classes.googleButton)} onClick={() => {console.log(handleSignUp());enableoverlayScreen(true);}}>
          <img src={process.env.PUBLIC_URL + '/img/Google_Logo.svg'} className={classes.gmailSvg} alt="Gmail Icon" />
          Sign With Google
        </Button>
