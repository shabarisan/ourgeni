...
import '../serviceProvider/serviceProviderDetailsStyle.css';
...
const EducationProviderDetails = (props: any) => {
    const classes = categoryHomeStyles();
    const { service_id } = useParams<ParamTypes>();
    const [providerData, setProviderData] = useState<any>()
    const [writeReview,SetWriteReview] = useState<any>()
    ...
    const fetchProvidersDetails = async () => {
        setLoder(true)
        api.fetchEducationListDetails(service_id)
            .then((response) => {
                setLoder(false)               
    ...
    return (
        <Grid container>
            ...
            providerData ?
                ...
                :
                <Box className="loder">
                    <Typography variant='h4'>No Education providers found</Typography>
                </Box>
            }
        </Grid>
    )
